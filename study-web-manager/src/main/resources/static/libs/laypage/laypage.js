...
//分页视图
    Page.prototype.view = function () {
        ...
        return '<div name="laypage' + laypage.v + '" class="laypage_main laypageskin_' + (conf.skin ? (function (skin) {
                ...
                return conf.skip
                    ? '<span class="laypage_total"><label>&#x5230;&#x7B2C;</label><input type="number" min="1" max="'+conf.pages+'" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>'
            ...
            +'<label>共</label>'
            +'<label class="laypage_curr">'+conf.items+'</label>'
            +'<label>条</label></span>'
            + '</div>'
            ;
    };
...
function loadPageData(divId, url, pages, params,isFirst) {
    if (!pages) {
        return;
    }
            ...
            var op={
                ...
                jump: function (obj, first) {
                    if (!first) {
                        ...
                        loadPageData(divId, url, _pages, params,false);
                    }
                },
...
function loadPage(url,cur,pageSize,pages,itmes,divId,curForm) {
    var op={
        cont: divId,
        changePageSize: true,
        pages: pages,
